...
<div class="pr-cat-card h-100" data-testid="card-animal-{{ $animal->id }}">
    <div class="pr-cat-card-img">
        <img src="{{ asset($animal->main_photo) }}" alt="{{ $animal->name }}" loading="lazy">
        @auth
    ...
    <div class="pr-cat-card-body">
        <div class="d-flex justify-content-between align-items-start mb-1">
            <h6 class="pr-cat-card-name m-0" data-testid="text-animal-name-{{ $animal->id }}">{{ $animal->name }}</h6>
            <small class="pr-muted" data-testid="text-animal-age-{{ $animal->id }}">{{ $animal->ageLabel() }}</small>
        </div>
        <div class="pr-muted small mb-2" data-testid="text-animal-breed-{{ $animal->id }}">{{ $animal->breed }}</div>
        <div class="pr-muted small mb-3">
            <i class="bi bi-geo-alt"></i>
            <span data-testid="text-animal-location-{{ $animal->id }}">{{ $animal->shelter->shelter_name ?? '' }}, {{ $animal->shelter->city ?? '' }}</span>
        </div>
        <a href="{{ route('animals.show', $animal) }}"
           class="btn pr-btn-outline w-100 btn-sm mt-auto"
           data-testid="link-detail-{{ $animal->id }}">Lihat Detail</a>
    </div>
</div>
