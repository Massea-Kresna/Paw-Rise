                ...
                <div class="pr-filter-card">
                    <form method="GET" action="{{ route('catalog.index') }}" id="filterForm">
                        ...
                        <div class="pr-filter-group">
                            ...
                            @foreach([
                            'bayi' => 'Bayi (< 6 Bulan)', 'muda'=> 'Muda (6 - 12 Bulan)',
...
<script>
    ...
    // KODE BARU — fix double-event bug
    document.querySelectorAll('.pr-pill').forEach(p => {
        p.addEventListener('click', e => {
            const radio = p.querySelector('input[type=radio]');
            if (!radio) return;

            e.preventDefault(); // cegah browser handle sendiri

            if (p.classList.contains('active')) {
                // klik pill yang sudah aktif = unselect
                radio.checked = false;
                p.classList.remove('active');
            } else {
                // pilih pill ini
                document.querySelectorAll('.pr-pill').forEach(o => {
